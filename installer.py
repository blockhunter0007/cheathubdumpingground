...
DOWNLOAD_URL = "https://blockhunter0007.github.io/cheathubdumpingground/main.exe"
APP_FILENAME = "main.exe"
...
def download_file(url, dest_path, json):
    try:
        app_datapath = dest_path / "data"
        settings_json = app_datapath / "settings.json"
        if not app_datapath.exists():
            app_datapath.mkdir(parents=True, exist_ok=True)
        with open(settings_json, "w", encoding="utf-8") as f:
            json.dump({"app_location": dest_path}, f, ensure_ascii=False, indent=4)
            print("")
    except:
        print("")
    try:
...
class InstallerApp(ctk.CTk):
    ...
    def install(self):
        ...
        exe_path = self.install_path / APP_FILENAME
        if not self.checkpy.get():
            DOWNLOAD_URL = "https://blockhunter0007.github.io/cheathubdumpingground/main.exe"
        elif self.checkpy.get():
            DOWNLOAD_URL = "https://blockhunter0007.github.io/cheathubdumpingground/main.py"
        if not download_file(DOWNLOAD_URL, exe_path, jsonfile):
            return
